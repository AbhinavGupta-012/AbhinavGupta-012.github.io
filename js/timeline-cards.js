// timeline-cards.js - Enhanced with new card design elements
const timelineData = [
    {
        year: 2014,
        months: [
            { name: "March", events: "Started my computer science degree at University." },
            { name: "July", events: "Summer internship at Tech Solutions Inc." }
        ]
    },
    {
        year: 2015,
        months: [
            { name: "January", events: "Joined the Programming Club. Participated in first hackathon." },
            { name: "May", events: "Completed advanced algorithms course with distinction." },
            { name: "October", events: "Started part-time work as a web developer." }
        ]
    },
    {
        year: 2016,
        months: [
            { name: "February", events: "Launched my first mobile app on Play Store." },
            { name: "June", events: "Study abroad program in Japan for 3 months." },
            { name: "November", events: "Received 'Student of the Year' award in CS department." }
        ]
    },
    {
        year: 2017,
        months: [
            { name: "April", events: "Internship at Google. Worked on AI research project." },
            { name: "August", events: "Graduated with honors. Bachelor of Science in Computer Science." },
            { name: "September", events: "Started full-time position as Software Engineer at TechCorp." }
        ]
    },
    {
        year: 2018,
        months: [
            { name: "March", events: "Promoted to Senior Software Engineer." },
            { name: "July", events: "Keynote speaker at DevCon 2018." }
        ]
    },
    {
        year: 2019,
        months: [
            { name: "January", events: "Started master's degree part-time while working." },
            { name: "June", events: "Published research paper on machine learning applications." }
        ]
    },
    {
        year: 2020,
        months: [
            { name: "April", events: "Led development of pandemic response software solution." },
            { name: "October", events: "Received 'Innovator of the Year' industry award." }
        ]
    },
    {
        year: 2021,
        months: [
            { name: "February", events: "Launched my own startup company." },
            { name: "August", events: "Secured $2M in venture capital funding." }
        ]
    },
    {
        year: 2022,
        months: [
            { name: "May", events: "Expanded company to international markets." },
            { name: "December", events: "Featured in '30 Under 30' tech entrepreneurs list." }
        ]
    },
    {
        year: 2023,
        months: [
            { name: "March", events: "Company reached 1 million users milestone." },
            { name: "September", events: "Merged with larger tech company, stayed on as CTO." }
        ]
    },
    {
        year: 2024,
        months: [
            { name: "January", events: "Started AI research initiative with university partners." },
            { name: "June", events: "Keynote at World Tech Summit in San Francisco." }
        ]
    }
];

let currentYearIndex = 0;
const foldedYearsContainer = document.getElementById('foldedYears');
const timelineYearsContainer = document.getElementById('timelineYears');
const monthsContainer = document.getElementById('monthsContainer');
const backButton = document.getElementById('backButton');
const backToYearButton = document.getElementById('backToYear');

const foldedYearsMap = new Map();

function initTimeline() {
    createYearCard(timelineData[currentYearIndex], true);

    backToYearButton.addEventListener('click', () => {
        monthsContainer.style.display = 'none';
        timelineYearsContainer.style.display = 'flex';
        backButton.style.display = 'none';
    });
}

function createYearCard(yearData, isActive = false) {
    // Remove any existing active card first
    if (isActive) {
        const existingActive = document.querySelector('.year-card:not(.folded)');
        if (existingActive) {
            existingActive.remove();
        }
    }

    // Check if we already have a folded card for this year
    if (!isActive && foldedYearsMap.has(yearData.year)) {
        return foldedYearsMap.get(yearData.year);
    }

    const yearCard = document.createElement('div');
    yearCard.className = isActive ? 'year-card' : 'year-card folded';
    yearCard.setAttribute('data-year', yearData.year);

    // Add SVG background
    const svg = document.createElement('div');
    svg.className = 'card-svg';
    svg.innerHTML = `
        <svg preserveAspectRatio="xMidYMid meet" viewBox="0 0 8334 8334" xmlns="http://www.w3.org/2000/svg">
            <g stroke="none" fill="#a89bff" transform="translate(0,8334) scale(0.1,-0.1)">
                ...
                <path d="M32163 52279 c-471 -68 -933 -380 -1295 -875 -656 -900 -947 -2298
    ...
    `;
    yearCard.appendChild(svg);

    // Add wave animations for active cards
    if (isActive) {
        for (let i = 0; i < 3; i++) {
            const wave = document.createElement('div');
            wave.className = 'wave';
            yearCard.appendChild(wave);
        }
    }
    ...
    yearCard.appendChild(borderTop);

    const yearBox = document.createElement('div');
    yearBox.className = 'year-box';
    yearBox.textContent = yearData.year;
    yearCard.appendChild(yearBox);

    // In the createYearCard function, replace the buttons section with:
    if (isActive) {
    ...
    openBtn.addEventListener('click', () => openYear(yearData));
    ...
    yearCard.appendChild(buttons);

    if (currentYearIndex > 0) {
        const closeBtn = document.createElement('button');
        closeBtn.className = 'close-card';
        closeBtn.innerHTML = '×';
        closeBtn.addEventListener('click', closeActiveYear);
        yearCard.appendChild(closeBtn);
    }

    // Ensure container refresh
    timelineYearsContainer.innerHTML = '';
    timelineYearsContainer.appendChild(yearCard);
}
 else {
        yearCard.addEventListener('click', () => setActiveYear(yearData.year));
        foldedYearsContainer.appendChild(yearCard);
        foldedYearsMap.set(yearData.year, yearCard);
    }

    highlightActiveFoldedYear();
    updateArrows();
    return yearCard;
}

function setActiveYear(year) {
    const index = timelineData.findIndex(d => d.year === year);
    if (index === -1) return;
    ...
    // Hide folded years when active card is shown
    foldedYearsContainer.style.opacity = '0.3';
    foldedYearsContainer.style.pointerEvents = 'none';
}

function closeActiveYear() {
    // Show folded years again
    foldedYearsContainer.style.opacity = '1';
    foldedYearsContainer.style.pointerEvents = 'auto';

    // Hide the active year card
    timelineYearsContainer.innerHTML = '';
    timelineYearsContainer.style.display = 'none';
}

function nextYear() {
    if (currentYearIndex >= timelineData.length - 1) {
        // If we're at the last year, loop back to the first
        currentYearIndex = 0;
    } else {
        currentYearIndex++;
    }

    // Create folded card for the previous year if it doesn't exist
    const prevYearIndex = currentYearIndex === 0 ? timelineData.length - 1 : currentYearIndex - 1;
    if (!foldedYearsMap.has(timelineData[prevYearIndex].year)) {
        createYearCard(timelineData[prevYearIndex], false);
    }

    // Create active card for the current year
    createYearCard(timelineData[currentYearIndex], true);
}
...
function openYear(yearData) {
    ...
    yearData.months.forEach(month => {
        ...
        monthFront.className = 'month-front';
        monthFront.innerHTML = `<h3>${month.name}</h3>`;

        const monthBack = document.createElement('div');
        monthBack.className = 'month-back';
        monthBack.textContent = month.events;

        monthInner.appendChild(monthFront);
        monthInner.appendChild(monthBack);
        monthCard.appendChild(monthInner);
...
function highlightActiveFoldedYear() {
    foldedYearsContainer.querySelectorAll('.year-card.folded').forEach(y => y.classList.remove('active-folded'));
    const activeFolded = foldedYearsContainer.querySelector(`.year-card.folded[data-year='${timelineData[currentYearIndex].year}']`);
    if (activeFolded) activeFolded.classList.add('active-folded');
}
